...
export default function DashboardTopbar({ onMenuClick }: DashboardTopbarProps) {
  ...
  const unreadCount = (notifData?.unreadCount || 0) + realtimeUnreadCount;
    ...
    <header className="h-16 flex-shrink-0 bg-[var(--surface)] border-b border-[var(--border)] flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-10 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link 
          ...
          title="Notifications"
        >
          <Bell className={`w-5 h-5 ${realtimeUnreadCount > 0 ? 'animate-[shake_0.5s_ease-in-out]' : ''}`} />
          {unreadCount > 0 && (
            <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-[var(--status-critical)] rounded-full border-2 border-[var(--background)] animate-pulse" />
          )}
        </Link>
      </div>
